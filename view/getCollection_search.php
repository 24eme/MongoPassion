...
if(isset($_SESSION['recherche_id']) and isset($_SESSION['recherche_g'])){
	...
	if($_SESSION['recherche_id']=="" and $_SESSION['recherche_g']=="field = content[...]"){echo "\"Aucun critère\"";}
	if($_SESSION['recherche_id']!=""){echo "\"".$_SESSION['recherche_id']."\"";}
	if($_SESSION['recherche_id']!="" and $_SESSION['recherche_g']!="field = content[...]"){echo " et ";}
	if($_SESSION['recherche_g']!="field = content[...]"){echo "\"".$_SESSION['recherche_g']."\"";}
	echo "</h1>";
}
else{
...
<nav>
	<div id="options">
		<span>
			<?php echo '<a href="index.php?action=createDocument&search='.$page.'">Nouveau Document</a>'; ?>
		</span>
	</div>
</nav>
...
<footer>
	...
	for ($i=1;$i<=$nbPages;$i++) {
		echo '<a href="index.php?action=getCollection_search&page='.$i.'">'.$i.'</a>';
		if($i!=$nbPages){echo '-';}
	}
	?>
</footer>
